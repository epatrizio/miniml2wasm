...
let rec typecheck_expr (loc, typ, expr') : (expr, _) result =
  ...
  | Eblock block ->
    ...
    Ok (loc, typ_b, Eblock block)
  | Eif (e_cond, e_then, e_else) ->
    let* l_cond, typ_cond, e_cond' = typecheck_expr e_cond in
    begin
      match typ_cond with
      | Tbool ->
        let* l_then, typ_then, e_then' = typecheck_expr e_then in
        let* l_else, typ_else, e_else' = typecheck_expr e_else in
        begin
  ...
  | Elet (stmt_assign, expr) ->
    let* stmt_assign = typecheck_stmt stmt_assign in
    let* l, t, expr' = typecheck_expr expr in
    Ok (loc, t, Elet (stmt_assign, (l, t, expr')))
  | Estmt stmt ->
    let* stmt = typecheck_stmt stmt in
    Ok (loc, Tunit, Estmt stmt)

and typecheck_block block : (typ * block, _) result =
  match block with
  | Bexpr expr ->
    let* loc, typ, expr' = typecheck_expr expr in
    Ok (typ, Bexpr (loc, typ, expr'))
  | Bseq (expr, block) -> (
    let* loc, typ, expr' = typecheck_expr expr in
    match typ with
    | Tunit ->
      let* t, b = typecheck_block block in
      Ok (t, Bseq ((loc, Tunit, expr'), b))
    | _ -> error loc "unit type expected in the left-hand side of a sequence" )

and typecheck_stmt (loc, stmt') : (stmt, _) result =
  match stmt' with
...
and typecheck_prog prog = typecheck_expr prog
